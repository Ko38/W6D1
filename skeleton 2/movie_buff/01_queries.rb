...
def harrison_ford
 ...
 # Actor.select('movies.id, title').joins(:castings, :movies).where("actors.name ='Harrison Ford' AND castings.ord != 1")
...
def directed_by_one_of(them)
  ...
  Actor.joins(:directed_movies).select('movies.id, title')
    .where(name: them)
end
...
def movie_names_before_1940
  ...
  Movie.select(:title).where('yr < 1940').distinct.pluck(:title)
end
